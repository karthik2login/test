...
using GithubResearch.Migrations.SeedData;
...
{
    public abstract class GithubResearchTestBase : AbpIntegratedTestBase<GithubResearchTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GithubResearchTestBase()
        {
        ...
        protected void UsingDbContext(Action<GithubResearchDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GithubResearchDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GithubResearchDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GithubResearchDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GithubResearchDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GithubResearchDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GithubResearchDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GithubResearchDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GithubResearchDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GithubResearchDbContext, Task<T>> func)
        {
            T result;
